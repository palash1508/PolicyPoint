    ...
    <main class=" mx-auto flex flex-col lg:flex-row gap-16 items-start p-3 md:p-14 bg-[#EEEEEE33]">
        <section class="flex-1 lg:px-0 w-full">
            <div class="flex flex-col w-full">
                <div class="">
                    <div class="">
                        <FinalFormStepper :steps="['Personal details', 'Medical history', 'Contact details']"
                            :currentStep="currentStep" />
                    </div>
                    <div v-if="currentStep === 1">
                        <FinalFormTravelPersonalDetail @next="goToStep(2)" @update:travellers="handleTravellersUpdate" />
                    </div>
                    <div v-else-if="currentStep === 2">
                        <FinalFormTravelMedicalHistory :travellers="members" @next="goToStep(3)" />
                    </div>
                    <div v-else-if="currentStep === 3">
                        <FinalFormTravelContactDetails />
                    </div>
                </div>
...
const handleTravellersUpdate = (travellers) => {
  // Handle updated travellers data
